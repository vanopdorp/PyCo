...
public:
    ...
    Value(std::function<Value()> fn) : data(fn) {}
    Value(Value (*fn)()) : data(std::function<Value()>(fn)) {}
    Value(std::function<Value(Value)> fn) : data(std::in_place_type<std::function<Value(Value)>>, fn) {}
    template<typename F>
    Value(F fn, std::enable_if_t<
        std::is_invocable_r_v<Value, F, Value>, int> = 0)
        : data(std::function<Value(Value)>(fn)) {}
    ...
    bool isFunc() const {
        return std::holds_alternative<std::function<Value()>>(data) || 
            std::holds_alternative<std::function<Value(Value)>>(data);
    }


    // Functie om de functie in Value te extraheren
    std::function<Value()> asFunc() const {
        // Controleer of de Value een functie is zonder argumenten
        if (auto p = std::get_if<std::function<Value()>>(&data)) {
            return *p;  // Retourneer de functie direct
        }

        // Controleer of de Value een functie is met één argument (Value)
        if (auto p = std::get_if<std::function<Value(Value)>>(&data)) {
            // Zet de functie om naar een lambda die geen argumenten verwacht
            return [p]() -> Value {
                // Het aanroepen van de functie is hier niet nodig, maar kan later gedaan worden
                throw std::runtime_error("Function requires an argument to be called");
            };
        }
    ...
    int asInt() const {
        ...
        if (auto p = std::get_if<double>(&data)) return static_cast<int>(*p);
        throw std::runtime_error("Value is not int");
    }

    Value operator()() const {
        if (auto p = std::get_if<std::function<Value()>>(&data)) {
            return (*p)();  // If it's a function with no arguments, call it.
        }
        
        if (auto p = std::get_if<std::function<Value(Value)>>(&data)) {
            // If it's a function with one argument, we need to forward the argument
            throw std::runtime_error("Function requires an argument to be called");
        }
        throw std::runtime_error("Value is not a function");
    }
    ...
    Value operator()(Args&&... args) const {
        ...
        throw std::runtime_error("Value is not a function with this arity");
    }


    double asDouble() const {
        if (auto p = std::get_if<double>(&data)) return *p;
        if (auto p = std::get_if<int>(&data)) return static_cast<double>(*p);
...
Value dict_get(Value& dictVal, const Value& key, const Value& default_value = Value()) {
    ...
    // Als de sleutel niet gevonden is, geef de standaardwaarde terug
...
inline std::vector<Value> iterate(const Value& v) {
    ...
    // Check of v een lijst is
    if (v.isList()) {
        result = v.asList()->items;
    }
    // Check of v een tuple is
    else if (v.isTuple()) {
        result = v.asTuple()->items;
    }
    // Check of v een string is
    else if (v.isString()) {
        std::string str = v.asString();
        for (char c : str) {
            result.push_back(Value(std::string(1, c)));  // Voeg elk karakter als een string toe aan de vector
        }
    }
    // Indien geen van bovenstaande, gooien we een uitzondering
    else {
        throw std::runtime_error("Value is neither a List, Tuple, nor String");
    }
...
Value is_op(const Value& a, const Value& b) {
    // Check if both are null (None in Python)
    if (a.isNull() && b.isNull()) return Value(true);

    // If both values are the same type, compare their data
    if (a.isObject() && b.isObject()) {
        return Value(a.asObject() == b.asObject()); // Compare shared_ptr directly
    }

    // If both values are lists, compare their data
    if (a.isList() && b.isList()) {
        return Value(a.asList() == b.asList()); // Compare shared_ptr directly
    }

    // If both values are tuples, compare their data
    if (a.isTuple() && b.isTuple()) {
        return Value(a.asTuple() == b.asTuple()); // Compare shared_ptr directly
    }

    // If both values are dicts, compare their data
    if (a.isDict() && b.isDict()) {
        return Value(a.asDict() == b.asDict()); // Compare shared_ptr directly
    }

    // Default: If types differ, they are not "the same object"
    return Value(false);
}
Value in_op(const Value& a, const Value& b) {
    if (b.isList()) {
        auto list = b.asList();
        for (auto& item : list->items) {  // Accessing items via the vector inside the shared_ptr
            if (eq(item, a).asBool()) {
                return Value(true);
            }
        }
    ...
    if (b.isDict()) {
        return Value(b.asDict()->contains(a));
    }

    if (b.isString()) {
        return Value(b.asString().find(a.asString()) != std::string::npos);
    }

    throw std::runtime_error("Unsupported operand for 'in'");
}

Value chr(const Value& v) {
    int code = v.asInt();
...
Value join_builtin_func(const Value& delimiter, const Value& list) {
    // Veronderstel dat list een std::shared_ptr is van een List-achtig object
    std::shared_ptr<List> list_data = list.asList();

    // Maak een vector van Value's handmatig
    std::vector<Value> vec_data;
    for (auto& item : list_data->items) {  // Directe toegang tot 'items' vector in List
        vec_data.push_back(item);
    }

    std::string result;
    for (size_t i = 0; i < vec_data.size(); ++i) {
        result += vec_data[i].asString();  // Of een andere manier om de stringrepresentatie van de Value op te halen
        if (i < vec_data.size() - 1) {
            result += delimiter.asString();  // Voeg de delimiter toe tussen de elementen
        }
    }
    return Value(result);
}
