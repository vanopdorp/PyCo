...
class CppCompiler(ast.NodeVisitor):
    ...
    def visit_ClassDef(self, node: ast.ClassDef):
        ...
        # Voeg standaard dunder-methodes toe als ze niet bestaan
        default_dunders = {
            "__new__": [
    ...
    def compile_if(self, node: ast.If):
        ...
        cond = f"{cond}.asBool()"  # ← voeg dit toe
    ...
    def compile_subscript_assign(self, node):
        # node.targets[0] is een ast.Subscript
        target = node.targets[0]
        obj, _ = self.compile_expr(target.value)
        index, _ = self.compile_expr(target.slice)
        val, _ = self.compile_expr(node.value)

        # Bepaal type van de variabele
    ...
    def compile_assign(self, node):
        # Check if the target is a Subscript (e.g., lst[0] = value)
    ...
    def generate_cpp(self):
        out = ['#include <iostream>', '#include "value.hpp"', '#include "range.hpp"','#include "booleans.hpp"','#include "builtin.hpp"']
        out += self.functions
        out.append("int main() {")
